...
from .graph_compiler   import compile_graph, emit_inference_runner_c
...
def generate(
    ...
    renders: list[tuple[str, str]] = [
        ...
        # inference_runner.c generated by graph_compiler (not Jinja2)
    ...
    # inference_runner.c -- CMSIS-NN graph compiler (zero external dependencies)
    try:
        plan     = compile_graph(src)
        runner_c = emit_inference_runner_c(plan, target_id=target.id, rtos=rtos)
        (out / 'inference_runner.c').write_text(runner_c, encoding='utf-8')
        files_written.append('inference_runner.c')
        # Pass scratch size into arena config for memory_config.h template
        arena.scratch_bytes = plan.scratch_bytes
    except Exception as e:
        raise CodegenError(f'Graph compiler failed: {e}') from e
